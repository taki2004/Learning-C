...
int work_on_input(void) {
    ...
    while (1) {
        ...
        int result = scanf(" %255s", buffer); //Temp input
...
int random_choice() {
    int random_num = rand() % (2 + 1); // Generate a random number between 0 and 2
    switch (random_num) {
        case 0:
            player2 = Rock;
            break;
...
void another_round(void) {
    printf("Do you fancy another round? (y for yes) (n Stop): ");
    char round;
    scanf("%s", &round);
    round = tolower(round);
    if (round == 'y') {
